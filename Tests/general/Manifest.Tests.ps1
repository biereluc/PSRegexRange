BeforeDiscovery {
    ...
    $moduleRoot = $Global:moduleRoot
    $manifestFileName = "$moduleName.psd1"
    $manifestPath = Get-ChildItem -Path (Join-Path -Path $moduleRoot -ChildPath $manifestFileName) # convert to path
    # Using Import-PowerShellDataFile over Test-ModuleManifest as it's easier to navigate
    $manifest = Import-PowerShellDataFile -Path $manifestPath
}

Describe 'Validating the module manifest' -ForEach @{ moduleRoot = $moduleRoot; manifest = $manifest; manifestPath = $manifestPath } {
