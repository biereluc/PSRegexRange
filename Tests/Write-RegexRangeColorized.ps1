...
function Write-RegexRangeColorized
{
    ...
    $rangeStart..$rangeEnd | ForEach-Object {
        ...
        if (($match.Success -and $currentValue -eq $match.Value) -or (Test-InRegexRange -Number $currentValue -RegexRange $regex))
        {
...
Write-RegexRangeColorized -Min 1 -Max 55 -Regex '[1-9]|[1-4][0-9]|50' -Wait -Boundary 10 -Verbose